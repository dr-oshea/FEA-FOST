...
CASE = 96;   % choose boundary conditions from previously set-up cases.
...
type = 2;           % Type of element: 1 = 2D quad, 2 = 3D Brick
...
theta1 = 0;     % [deg]
theta2 = 0;     % [deg]
layers = 1;     % theta1 and theta2 give orientation of material for case of multiple layers (layers > 1)

% Discretisation
mesh_div = 2;       % element divisions along x-dimension (2D), z-dim (3D)
aspect = 1;         % aspect ratio of elements (1 = square/cubic)
...
if MAT == 4
    ...
    if model == 1   % Myocardium model (2022)
        L00 = 1000000;  % this needs to be orders of magnitude larger than other values (enforces incompressibility)
        LFF = 1490;
        LSS = 358;
        LNN = 80;
        M00 = 1.1;
        ...
        kappaS = 0.0547;
        k = 2;
        input.model = [model, L00, LFF, LSS, LNN, M00, MFS, kappaF, kappaS, k];
    end
end
PLANAR = 2; % 1 = Plane stress, 2 = Plane strain;

% Specified final loading (Force OR Stress used, depending on problem)
N = 200;        % Load steps (increasing takes longer, though more stable solution process)   CHANGED LOAD STEP FROM 250 to 10 to just get it running
Force = 0.0;      % [Newtons]
Stress = 0.5;      % [should match units of material parameters, kPa, MPa, etc..]
...
alpha2 = 10^-2; % tolerance on process modifications (not relevant)
maxMOD = 10;    % maximum number of process "modifications" before exiting
maxITER = 300;  % maximum number of iterations before exiting 
...
% INSTRUCTIONS
% Create 6 of these (91,..,96), one for each defomration mode. For each,
% create a CCASE and FCASE number, then in input_constraint and input_load
% files, create the required CCASE and FCASE algorithms to correctly place
% the loadings
elseif CASE == 91
...
input.EL = input_element(input.ND,mesh_div,aspect,layers,xdim,ydim,zdim,theta1,theta2,type);
...
% Additional info
numN = size(input.ND,1);    % total nodes in defined problem
dofT = numN*dofN;           % total degrees of freedom in defined problem
numE = size(input.EL,1);    % total elements in defined problem
...
while n <= N
    ...
    elseif (GRAD == 3 && n > 1)
        ...
        K = Assemble_Tangent_Stiffness(Q,input,type,ndsE,dofN,PROG);
        if n == 5
            hello=1;
        end
    end
...
%     fprintf('\n         Min. Diagonal = %.2d',min(diag(full(K))));
   ...
   [Q,P,FAIL,iter,normR] = NonlinearIteration(input,Q,P,Pn,K,L,dofT,ndsE,alpha1,maxITER,maxMOD,PROCMOD,FAIL,PROG);  
...
if type == 1
    [F,~,~,~,S,tau,P,~] = postNonlin_2D(input,Qsol,Psol,ele_analyse);
elseif type == 2
    [F,~,~,~,S,tau,P,~] = postNonlin_3D(input,Qsol,Psol,ele_analyse);
end
elapsedTime2 = toc;
fprintf('\n   Completed!')
...
lamx = zeros(N,1); tauxx = zeros(N,1); Pxx = zeros(N,1); Sxx = zeros(N,1);
lamy = zeros(N,1); tauyy = zeros(N,1); Pxy = zeros(N,1); Syy = zeros(N,1);
gamxy = zeros(N,1);
for i = 1 : N
    ...
    % Get S11 from each element
    v1 = S(el1,numGrad*(1-1)+1,i);
    v2 = S(el2,numGrad*(2-1)+1,i);
    v3 = S(el3,numGrad*(3-1)+1,i);
    v4 = S(el4,numGrad*(4-1)+1,i);
    Sxx(i) = mean([v1 v2 v3 v4]);

    % Get S22 from each element
    v1 = S(el1,numGrad*(1-1)+2,i);
    v2 = S(el2,numGrad*(2-1)+2,i);
    v3 = S(el3,numGrad*(3-1)+2,i);
    v4 = S(el4,numGrad*(4-1)+2,i);
    Syy(i) = mean([v1 v2 v3 v4]);
...
plot(0.5*(lamx.^2-1),Sxx,'-o');
plot(0.5*(lamy.^2-1),Syy,'-x');
xlabel('E_{xx}, E_{yy}')
...
%% STRESS COUNTOURS
if type == 1 && PLOT ~= 0
Xloc = reshape(input.ND(:,2),[mesh_div+1,numel(input.ND(:,1))/(mesh_div+1)]);
Yloc = reshape(input.ND(:,3),[mesh_div+1,numel(input.ND(:,1))/(mesh_div+1)]);
