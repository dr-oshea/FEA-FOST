...
elementType = 1;           % Type of element: 1 = 2D quad, 2 = 3D Brick
...
if MAT == 4
    ...
    elseif constitutiveModel == 3      % Gultekin model (2017)
        E = 10;
        v = 0.3;
        input.model = [3, E, v];
...
numLoadSteps = 100;        % Load steps (increasing takes longer, though more stable solution process)   CHANGED LOAD STEP FROM 250 to 10 to just get it running
...
SolverOptions.QUASI_ITER = 1; % number of iterations before new tangent stiffness calculation
...
SolverOptions.lineSearchOptions.use = 0;
...
SolverOptions.MIXED = 0;    % Use mixed formulation for incompressibility
