...
CASE = 3;
MAT = 1;    % 1 = specify isotropic, 2 = specify orthotropic, 3 = known sets
...
%     Ciarletta2009 = [0.0237;0.0918;0.0059;1.3815;1.1549;0.0016;0.0002;0.0004;0.0004];
    Ciarletta2009 = get_MatProps([[1;1;1].*0;[0.01816;0.0846;0.00803;0.0005;0.0007;0.0007]]);
...
%     maxTar = maxTar + maxInc;
    fprintf('\n      Maximum Target Load:    %.3f N',max(full(Pn)))
    
    % Initial Residual = Target Load - Previous Load = deltaP
    R = DeltaP;
    
    % Initial Norm
    normR = norm(R(L))/max(Pn);
...
%     fprintf('\n         Min. Diagonal = %.2d',min(diag(full(K))));
    ...
    while normR > alpha1 %|| norm(DeltaQ) > alpha1
        ...
        % Update stiffness using Broyden's method
        if BROYDEN == 1 && iter > 0
            y = P-Pp';
            s = Q-Qp;
            K = K + ((y-K*s)/norm(s))*transpose(s);
            Pp = P';
            Qp = Q;
        end
