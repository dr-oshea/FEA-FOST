function Ks = Assemble_Secant_Stiffness(Q,input,type,ndPerElem,dofN)

% This script assembles the secant stiffness matrix for at a given
% displacement field
...
% UPDATE LOG:
% 12/04/2018 - added compaibility for a 3D element
...
set(hwb,'Name','Assembling Global Secant Stiffness Matrix');
indj = 1;
...
for i=1:numElem
    ...
    if type == 1
        ...
        K0e = Element_Secant_Stiffness_2D(q,localCoords,theta,input.L0,input.M0);
    ...
    if type == 2
        ...
        % Determine element stiffness matrix
        K0e = Element_Secant_Stiffness_3D(q,localCoords,theta,input.L0,input.M0);
...
Ks = sparse(rowK,colK,valK,dofN*size(input.ND,1),dofN*size(input.ND,1));
